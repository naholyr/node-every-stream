...
function FTPReadStream (path, client) {
  ...
  this.on('end',   function () { this.ended = true })
  this.on('close', function () { this.ended = true })
  this.on('error', function () { this.ended = true })
}
 ...
 * Read Stream for FTP
 */
function FTPWriteStream (path, client) {
  // Default state
  this.writable = true
  this.ended = false
  this.stream = null
  ...
  this.on('pipe', function (stream) {
    ...
    client.put(stream, path, function (err) {
...
util.inherits(FTPWriteStream, Stream)

// Write is ignored here
FTPWriteStream.prototype.write = function (string) {
  console.log('FTP.WRITE', string.toString())
  return true
}
FTPWriteStream.prototype.end = function () {
  console.log('FTP.END')
  if (this.ended) return false
  this.emit('close')
}

FTPWriteStream.prototype.destroy = function () {
  if (this.ended) return false
  if (this.stream) this.stream.destroy()
  this.emit('close')
}
FTPWriteStream.prototype.destroySoon = function () {
  if (this.ended) return false
  if (this.stream) this.stream.destroySoon()
  this.emit('close')
}
...
FTPDriver.prototype.createWritable = function (cb) {
  ...
  self._write_streams++
  self.onReady(function () {
    console.log('READY WRITABLE')
    var stream = new FTPWriteStream(self.path, self.client)
    stream.on('close', function ()    { self._read_streams--; self.client.emit('close') })
    stream.on('error', function (err) { self._read_streams--; self.client.emit('error', err) })
    cb(undefined, stream)
  })
}
